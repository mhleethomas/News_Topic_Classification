...
├── data/
│   ├── raw/
│   │   └── bbc-text.csv          ← downloaded by download_data.py (not committed to git)
...
## Quick Start (run once, in order)

```bash
# 1. Install dependencies
pip install -r requirements.txt

# 2. Download the dataset (free, no account needed)
...
After step 3, all four CSV files will be in `data/processed/`.
